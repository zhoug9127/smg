...
async fn setup_streaming_internal_mcp_error_test() -> (
    MockFailingMCPServer,
    MockWorker,
    Box<dyn smg::routers::RouterTrait>,
    tempfile::TempDir,
) {
    let mcp = MockFailingMCPServer::start(TEST_INTERNAL_MCP_ERROR_MARKER)
        .await
        .expect("start failing mcp");
    let mcp_yaml = format!(
        "servers:\n  - name: {TEST_INTERNAL_MCP_SERVER_LABEL}\n    protocol: streamable\n    internal: true\n    url: {}\n",
...
#[expect(clippy::print_stdout, reason = "test diagnostic output")]
#[tokio::test]
async fn test_streaming_response_omits_internal_mcp_events() {
    ...
    let req = ResponsesRequest {
        ...
        text: None,
        user: None,
        request_id: Some("resp_streaming_internal_mcp_test".to_string()),
        priority: 0,
        frequency_penalty: Some(0.0),
        presence_penalty: Some(0.0),
        stop: None,
    ...
    println!("Streaming internal MCP SSE response:\n{body_text}");

    assert!(
        !body_text.contains(TEST_INTERNAL_MCP_SERVER_LABEL),
        "internal MCP server label should not appear in streaming output"
    );
    assert!(
        !body_text.contains("brave_web_search"),
        "internal MCP tool name should not appear in streaming output"
    );
    ...
    for (event_name, data) in &events {
        if let Some(event_name) = event_name {
            assert!(
                !event_name.starts_with("response.mcp_"),
                "internal MCP events should not be forwarded: {event_name} {data}"
            );
            assert!(
                !event_name.starts_with("response.function_call"),
                "internal MCP tool-call events should not be forwarded: {event_name} {data}"
            );
        }
        ...
        if let Some(item) = data.get("item") {
            ...
            assert!(
                ...
                "internal MCP tool items should not be forwarded: {data}"
            );
        }
    }
...
#[expect(clippy::print_stdout, reason = "test diagnostic output")]
#[tokio::test]
async fn test_streaming_response_hides_internal_mcp_error_details() {
    let (mut mcp, mut worker, router, _dir) = setup_streaming_internal_mcp_error_test().await;
    ...
    let req = ResponsesRequest {
        ...
        text: None,
        user: None,
        request_id: Some("resp_streaming_internal_mcp_error_test".to_string()),
        priority: 0,
        frequency_penalty: Some(0.0),
        presence_penalty: Some(0.0),
        stop: None,
    ...
    println!("Streaming internal MCP error SSE response:\n{body_text}");
    ...
    assert!(
        !body_text.contains(TEST_INTERNAL_MCP_ERROR_MARKER),
        "internal MCP error details should not appear in streaming output"
    );
